...
public class A06
{
    public static void main(String[] args)
    {
         ...
         */
        context.registerBean(AutowiredAnnotationBeanPostProcessor.class);// 使@Autowired生效
        context.registerBean(CommonAnnotationBeanPostProcessor.class); // 使@PostConstruct生效

        /*
         * 例2: Java 配置类在添加了 bean 工厂后处理器后, 配置类提前创建，BeanPostProcessor还未注册
         * -> 你会发现用传统接口方式的注入和初始化仍然成功, 而 @Autowired 和 @PostConstruct 的注入和初始化失败
         */

        context.registerBean(ConfigurationClassPostProcessor.class);// 使@Configuration生效
