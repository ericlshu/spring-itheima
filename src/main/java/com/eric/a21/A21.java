...
public class A21
{
    public static void main(String[] args) throws Exception
    {
        ...
        for (MethodParameter parameter : handlerMethod.getMethodParameters())
        {
            ...
            String annotation = annotations.length() > 0 ? " @" + annotations + " " : " ";
            String paramDetails = "[" + parameter.getParameterIndex() + "] "
                    + annotation + parameter.getParameterType().getSimpleName();

            // 多个解析器组合
            HandlerMethodArgumentResolverComposite composite = new HandlerMethodArgumentResolverComposite();
            composite.addResolvers(
                    new RequestParamMethodArgumentResolver(beanFactory, false), // false 表示有 @RequestParam才能解析
                    new PathVariableMethodArgumentResolver(),
                    new RequestHeaderMethodArgumentResolver(beanFactory),
                    new ServletCookieValueMethodArgumentResolver(beanFactory),
                    new ExpressionValueMethodArgumentResolver(beanFactory),
                    new ServletRequestMethodArgumentResolver(),
                    new ServletModelAttributeMethodProcessor(false),// 必须有@ModelAttribute才能解析
                    new RequestResponseBodyMethodProcessor(List.of(new MappingJackson2HttpMessageConverter())),
                    new ServletModelAttributeMethodProcessor(true), // 省略了@ModelAttribute也能解析
                    new RequestParamMethodArgumentResolver(beanFactory, true)  // true 表示省略 @RequestParam注解也可以默认解析
            );
            ...
            {
                // 支持此参数
                Object paramValue = composite.resolveArgument(parameter, container, new ServletWebRequest(request), factory);
                paramDetails = paramDetails + " " + parameter.getParameterName() + " -> " + paramValue;
                // @ModelAttribute解析完成后会将结果放入container对象中
                log.debug("模型数据为：{}", container.getModel());
            }

            log.info("paramDetails : {}", paramDetails);
        }
    }
